<template>
  <div>
    ...
    >
      ...
      <template v-slot:top>
        <v-toolbar flat>
          <v-toolbar-title>Gestion de grupos</v-toolbar-title>
    ...
    <v-dialog v-model="dialogUpdate" max-width="400" persistent>
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="black darken-1" text @click="updateGroupRemote()">
            Actualizar
          </v-btn>
        </v-card-actions>
      </v-card>
...
export default {
  ...
  methods: {
    ...
    updateGroupRemote() {
      API.updateGroup(this.dataModelUpdate).then((response) => {
        this.getGroupsRemote()
